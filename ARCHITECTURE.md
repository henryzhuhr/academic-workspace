...
| 项目正式产物 | 项目 `outputs/` | `reports/` 中的跨项目引用 |
...
`scripts/workspace.py` 是结构管理的唯一正式 CLI，通过 `uv run` 使用锁定的 Python 3.9+ 环境，脚本本身只依赖标准库，提供：

- `new`：从 `_template` 创建项目，与 dashboard 更新共同提交或共同回滚；
- `check`：只读验证工作区；
- `dashboard`：从权威元数据重建项目总览。

CLI 不读取或修改外部文献全文，不处理项目研究数据，也不替代项目自己的分析工作流。它只维护工作区级结构契约。
...
| 原始数据安全默认 | 避免误提交大型或敏感材料 | 项目必须维护数据登记和路径映射 |
| 稳定项目路径 | 保护引用、复现脚本和历史链接 | archived 项目仍保留在 `projects/` |
| 单一长期工作分支 | 保持跨项目知识和组合视图完整 | 分支不能充当项目目录 |
| uv 管理的标准库 CLI | 锁定 Python 版本并降低长期维护成本 | 正式命令统一使用 `uv run`；校验器保持轻量 Schema 子集 |
