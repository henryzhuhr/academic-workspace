...
- 项目专属论文、图表、幻灯片和交付物的源稿放入 `writing/` 或 `analysis/`；`outputs/` 保存产物登记、可复现命令和轻量元数据。
- 渲染后的 PDF 默认不进入 Git：可由 LaTeX/代码重建的产物重新生成，参考资料和不可再生文件进入外部文献库或项目外部存储。
- 跨项目总结、阶段汇总和按需报告放在 `reports/`；PDF、DOCX 等导出文件保留源稿和登记信息，成品放在外部存储。
